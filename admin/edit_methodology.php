...
include('dbcon.php');
$id = $_GET['id'];
$sqlqry = "select * from methodology where id='$id'";
...
<div class="content-wrapper">
    ...
    <div class="page-heading">
        <h1 class="page-title">methodology Page</h1>
        <ol class="breadcrumb">
    ...
    if (isset($_POST['submit'])) {
        ...
        $id = $_POST['id'];
        $sql_update = "update methodology set p1='$p1', p2='$p2', p3='$p3',p4='$p4',p5='$p5',p6='$p6',p7='$p7', p8='$p8' where id='$id'";
        $update = mysqli_query($con, $sql_update);



        if ($_FILES['image1']['name']) {
            $update = mysqli_query($con, "update methodology set image1='" . $_FILES['image1']['name'] . "' where id=" . $id);
        }
        if ($_FILES['image2']['name']) {
            $update = mysqli_query($con, "update methodology set image2='" . $_FILES['image2']['name'] . "' where id=" . $id);
        }
        if ($_FILES['image3']['name']) {
            $update = mysqli_query($con, "update methodology set image3='" . $_FILES['image3']['name'] . "' where id=" . $id);
        }
        if ($_FILES['image4']['name']) {
            $update = mysqli_query($con, "update methodology set image4='" . $_FILES['image4']['name'] . "' where id=" . $id);
        }
        if ($update) {
    ?>
            <script>
                alert('Data Updated Successfully');
                window.open('admin_methodology.php', '_self');
            </script>
    <?php
        }
    }
